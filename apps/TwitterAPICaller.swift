...
class TwitterAPICaller: BDBOAuth1SessionManager {    
    ...
    func postTweet(tweetString:String, success: @escaping () -> (), failure: @escaping (Error) -> ()) {
        ...
        TwitterAPICaller.client?.post(myUrl, parameters: ["status":tweetString], progress: nil, success: { (tasl:URLSessionDataTask, response:Any?) in
            success()
        }, failure: { (task:URLSessionDataTask?, error: Error) in
            failure(error)
        })
    ...
    func favoriteTweet(tweetId:Int, success: @escaping () -> (), failure: @escaping (Error) -> ()) {
        ...
        TwitterAPICaller.client?.post(myUrl, parameters: ["id":tweetId], progress: nil, success: { (tasl:URLSessionDataTask, response:Any?) in
            success()
        }, failure: { (task:URLSessionDataTask?, error: Error) in
            failure(error)
        })
